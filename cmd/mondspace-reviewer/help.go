...
var commands = []struct {
	...
	{
		"export", "Write the review up, or hand the findings store to another tool.",
		"msr export [--format=md|json|slack|agent|plan|markdown|github-issues|jsonl]",
		[]string{
			"--format=<f>     md (default), json or slack render one session's report;",
			"                 agent, plan, markdown, github-issues and jsonl render the",
			"                 findings store, which outlives the session",
			"--target=<id>    which review; default is the one open in `msr web`",
	...
	{
		"scan", "Run the installed analysers over a range and store what they found.",
		"msr scan [--since=<ref>] [--until=<ref>] [--repo=<path>]",
		[]string{
			"--since=<ref>    what to measure the change against (default HEAD)",
			"--until=<ref>    the far end of the range (default: the working tree)",
			"--branch=<b>     record against this branch (default: the one checked out)",
			"--all            include files .msrignore would keep out",
			"",
			"Findings are reconciled rather than re-raised: a second run over",
			"unchanged code writes nothing, a dismissal is never raised again, and",
			"a finding whose code has gone is closed.",
		},
	},
	{
		"findings", "List what stands, and record what you made of it.",
		"msr findings [list] | msr findings accept <id>... | msr findings dismiss <id>...",
		[]string{
			"--branch=<b>     which branch (default: the one checked out)",
	...
	{
		"push", "Hand a selection to an implementation agent as an ordered brief.",
		"msr push <id>... | msr push --batch --min-severity=high [--dry-run]",
		[]string{
			"--batch          push everything matching the filters, not named ids",
			"--state=<s>      with --batch, only items in this state",
			"--min-severity   with --batch, only findings at least this severe",
			"--to=<adapter>   file (default), stdout or clipboard",
			"--batch-id=<id>  reuse a batch id; a batch already sent sends nothing",
			"--dry-run        render the brief and send nothing",
		},
	},
	{
		"auto", "Let a judge push findings at checkpoints. Off by default.",
		"msr auto on | off | status | run [--session=<id>]",
		[]string{
			"on|off          turn auto-mode on or off; off takes effect at the next",
			"                checkpoint, and any manual `msr push` stands it down",
			"                for the rest of the session",
			"status          the policy, this session's pushes, decisions logged",
			"run             consider a push now — call it when the agent reaches",
			"                a checkpoint",
			"--session=<id>  which agent session, for the per-session budget",
			"--dry-run       decide and log, but send nothing",
			"",
			"The judge may only select, order and phrase findings that already",
			"exist. Every decision, taken or not, is written to judge.jsonl with",
			"its reason. Bounds live in .mondspace/auto.toml.",
		},
	},
	{
	...
	{
		"gc", "Tidy up: fold the findings file, drop throwaway review refs.",
		"msr gc [--session=<id>] [--repo=.] [--dry-run]",
		[]string{
			"--dry-run        print what would be done, do nothing",
			"--dir=<dir>      the shared directory (default <repo>/.mondspace)",
			"",
			"The findings file is append-only, so a finding raised, ruled on and",
			"pushed is three lines that mean one. This folds them, and deletes",
			"the review refs of sessions whose logs are gone.",
		},
	},
